...
pinn_sweep_mag.py
=================
Solver PINN para a cavidade cisalhante com efeito magnético (ferrofluido)
com suporte a varredura de parâmetros (sweep) via arquivo JSON.

Resolve as equações de Navier-Stokes 2D incompressíveis com força
de Kelvin (magnetização) acoplada via número magnético adimensional Mn.
...
------------------------------------------------------------------------------
  Varredura (sweep):
      python3 pinn_sweep_mag.py sweep_config_mag.json

  Simulação única:
      python3 pinn_sweep_mag.py params.json --single
...
------------------------------------------------------------------------------
PARÂMETROS MAGNÉTICOS NO JSON
------------------------------------------------------------------------------
  H0  : intensidade do campo magnético aplicado (A/m)
  chi : susceptibilidade magnética do fluido (adimensional)

  O campo H é calculado analiticamente como H(x,y) = H0 * (1 - y/D)
  e exportado como PNG e como campo adicional no VTK.

------------------------------------------------------------------------------
ESTRUTURA DO ARQUIVO
------------------------------------------------------------------------------
  PARTE 1 — SOLVER PINN MAGNÉTICO   (linhas ~95  – ~430)
    - build_net()
    - generate_interior()
    - generate_bc()
    - compute_loss()        ← inclui força de Kelvin em f_v
    - compute_H_field()     ← campo H analítico sobre o grid
    - export_results()      ← plota u, v, p e H; exporta VTK com H
    - plot_losses()
    - run_simulation()

  PARTE 2 — ORQUESTRADOR SWEEP      (linhas ~440 – ~540)
    - carregar_config()
    - montar_params()
    - main()
"""
...
#   ██      ██   ██ ██   ██    ██    ███████       ██
#
#   PARTE 1 — SOLVER PINN MAGNÉTICO
#   Cavidade cisalhante · Navier-Stokes 2D + força de Kelvin
#
# ==============================================================================
...
# Permeabilidade magnética do vácuo (constante física)
MU0 = 4.0 * np.pi * 1e-7
...
def build_net(n_layers: int, neurons: int, act_class, use_norm: bool) -> nn.Sequential:
    """Constrói MLP com LayerNorm opcional. Saída: (u, v, p)."""
    modules = []
...
# ------------------------------------------------------------------------------
# Função de perda — resíduo N-S magnético + condições de contorno
# ------------------------------------------------------------------------------
def compute_loss(model, x_f, x_bc, u_bc, v_bc, nu, H0, chi, D, weights):
    """
    Calcula perda total incluindo força de Kelvin no termo de momentum v:

        f_v = u*v_x + v*v_y + p_y - nu*(v_xx+v_yy) + Fmag
        Fmag = -Mn * (1 - y/D)
        Mn   = mu0 * chi * H0² * D

    Gradientes vetorizados: 1 passo por variável de 1ª ordem (era 2),
    4 passes para 2ª ordem — total de 7 passes (mínimo necessário para N-S).
    """
    w_f      = weights["w_f"]
    ...
    # --- Pontos interiores: resíduo das equações de N-S magnético ---
    ...
    # Coordenada y dos pontos interiores
    y = x_f[:, 1:2]

    # Número magnético adimensional e força de Kelvin
    Mn   = MU0 * chi * H0**2 * D
    Fmag = -Mn * (1.0 - y / D)

    # Resíduos: momentum x, momentum y (com Fmag), continuidade
...
# ------------------------------------------------------------------------------
# Campo H analítico sobre o grid de pós-processamento
# ------------------------------------------------------------------------------
def compute_H_field(Y: np.ndarray, H0: float, D: float) -> np.ndarray:
    """
    Calcula o campo magnético H(x,y) = H0 * (1 - y/D) sobre o grid 2D.
    O campo é uniforme em x e varia linearmente em y (aplicado externamente).
    Y : array meshgrid de coordenadas y, shape (N, N)
    Retorna H com mesmo shape de Y.
    """
    return H0 * (1.0 - Y / D)
...
# ------------------------------------------------------------------------------
def export_results(model, device, nome_caso: str, H0: float, chi: float, D: float):
    """
    Gera PNGs dos campos u, v, p e H, linhas de corrente e arquivo VTK.
    O campo H é calculado analiticamente e exportado junto com u, v, p.
    """
    ...
    # Campo H analítico
    H = compute_H_field(Y, H0, D)

    # PNGs: u, v, p e H
    for data, name, cmap, titulo in zip(
        [u,         v,         p,           H],
        ["u",       "v",       "pressao",   "H"],
        ["viridis", "viridis", "coolwarm",  "plasma"],
        ["Campo de u (velocidade horizontal)",
         "Campo de v (velocidade vertical)",
         "Campo de pressão",
         f"Campo magnético H  (H0={H0:.1f}, χ={chi})"],
    ):
    ...
    # VTK — inclui campo H como dado adicional de ponto
...
# ------------------------------------------------------------------------------
# Função principal do solver magnético
# ------------------------------------------------------------------------------
def run_simulation(params: dict) -> dict:
    """
    Executa uma simulação PINN magnética completa a partir de um dicionário.
    ...
    Parâmetros opcionais:
        ...
        D               (float) altura do domínio (padrão: 1.0)
    ...
    use_compile     = bool(params.get("use_compile",    False))
    ...
    # Número magnético adimensional (informativo no log)
    Mn = MU0 * chi * H0**2 * D
    ...
    if "output_dir" in params:
        ...
        nome_caso = f"Re{int(Re)}_H{int(H0)}_chi{chi}_N{N_int}_E{epochs}_{timestamp}"
    ...
    print(f"  H0={H0} | chi={chi} | Mn={Mn:.4e}")

    # --- Salva parâmetros completos da simulação ---
    with open(f"{nome_caso}/parametros.json", "w") as f:
        json.dump({
            **params,
            "activation":  act_name,
            "device":      str(device),
            "Mn_calculado": round(Mn, 6),
        }, f, indent=4)
    ...
    # --- Log em memória (flush periódico) ---
    # Colunas: loss_f | loss_u_top | loss_u_rest | loss_v | loss_total
    ...
    log_file.write(
        ...
        "u_avg u_std v_avg v_std H0 chi Mn\n"
    )
    ...
    for epoch in range(epochs):
        ...
        if isinstance(optimizer, torch.optim.LBFGS):
            def closure():
                ...
                _, _, _, _, loss_c = compute_loss(
                    model, x_f, x_bc, u_bc, v_bc, nu, H0, chi, D, weights
                )
                loss_c.backward()
                return loss_c
            ...
            with torch.no_grad():
                loss_f, loss_u_top, loss_u_rest, loss_v, loss_total = compute_loss(
                    model, x_f.clone().requires_grad_(True),
                    x_bc, u_bc, v_bc, nu, H0, chi, D, weights
                )
        else:
            optimizer.zero_grad()
        ...
        # Row inclui H0, chi e Mn como colunas fixas para rastreabilidade
        row = [
    ...
    export_results(model, device, nome_caso, H0, chi, D)
...
#   Lê sweep_config_mag.json e executa run_simulation() para cada run
#
# ==============================================================================
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python pinn_sweep_mag.py sweep_config_mag.json")
        print("     python pinn_sweep_mag.py params.json --single")
        sys.exit(1)
    ...
    sweep_name = config.get("sweep_name", "sweep_mag")
    base       = config["base"]
    runs       = config["runs"]
    n_total    = len(runs)
